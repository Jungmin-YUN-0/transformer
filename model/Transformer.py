...
class Transformer(nn.Module):
    def __init__(self, INPUT_DIM, OUTPUT_DIM, SRC_PAD_IDX, TRG_PAD_IDX, HIDDEN_DIM, ENC_LAYERS, DEC_LAYERS, ENC_HEADS, DEC_HEADS, ENC_PF_DIM, DEC_PF_DIM, ENC_DROPOUT, DEC_DROPOUT, device):
        ...
        #self.encoder_layer = nn.TransformerEncoderLayer(HIDDEN_DIM, ENC_HEADS, ENC_PF_DIM, ENC_DROPOUT)###
        #self.encoder = nn.TransformerEncoder(self.encoder_layer, ENC_LAYERS)###

        self.decoder = Decoder(OUTPUT_DIM, TRG_PAD_IDX, HIDDEN_DIM, DEC_LAYERS, DEC_HEADS, DEC_PF_DIM, DEC_DROPOUT, device)
        #self.decoder_layer = nn.TransformerDecoderLayer(HIDDEN_DIM, DEC_HEADS, DEC_PF_DIM, DEC_DROPOUT) ###
        #self.decoder = nn.TransformerDecoder(self.decoder_layer, DEC_LAYERS)###
